 ...
 */
Datum.prototype.estimateRewards = funtion() {
            ...
            .then(encryptedSecret => {
                ...
                return this.identity.signMsg(msg).then(signed => {
                    ...
                    if (results.indexOf(undefined) != -1) {
                        console.log('upload to server failed. check error console');
                    }
                    return results[0];
                })
            });
...
Datum.prototype.get = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        //try getting from second node (first is master)
        return Promise.all([
            this.storage.postCustomStorageNode(results[0][1], "/v1/storage/download", postParams),
            this.storage.postCustomStorageNode(results[0][1], "/v1/storage/downloadHaeder", postParams)]);
    }).then(results => {
        ...
        return datum.identity.decrypt(header);
    }).then(pass => {
...
Datum.prototype.getHeader = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        return this.storage.postCustomStorageNode(results[0][1], "/v1/storage/downloadHeader", postParams)
    });
};
