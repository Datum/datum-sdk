        ...
        .then(encryptedSecret => {
            ...
            return this.identity.signMsg(msg).then(signed => {
                ...
                for(var i = 0;i < results.length;i++) {
                    if(isURL(results[i])) {
                        prom.push(
                            this.storage.postCustomStorageNode(results[i], "/v1/storage/store", postParams)
                            .catch(function(err){
...
Datum.prototype.get = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        /*
        .then(res => {
            if(res === undefined) throw new Error('error downloading content');
            return res;
        });
        */
        /*
        .catch(error => {
            return this.storage.postCustomStorageNode(results[0][2], "/v1/storage/download", postParams)
        })
        .then(secondResponse => {
            return secondResponse;
        })
        .catch(error => {
            //get from master as last chance
            return this.storage.postCustomStorageNode(results[0][0], "/v1/storage/download", postParams)
            return Promise.all([results, this.identity.signMsg(msg)]);
        })
        */
     }).then(results => {
...
Datum.prototype.getPassword = function (id) {
...
Datum.prototype.addPublicKeyForData = function (id , pubKey) {
    ...
    this.getPassword(id).then(pass => {
        return this.encryptDataForPublicKeys(pass, pubKey);
    }).then(encHeader => {
