...
let registratorContract = new web3Static.eth.Contract(settings.contracts.registryContractABI, settings.contracts.registryContractAddress);
...
Datum.prototype.initialize = function (config) {
    ...
    storageCostsContract = new web3Static.eth.Contract(settings.contracts.storageCostsABI, settings.contracts.storageCostsAddress);
    storageContract = new web3Static.eth.Contract(settings.contracts.storageABI, settings.contracts.storageAddress);
    claimsContract = new web3Static.eth.Contract(settings.contracts.registryContractABI, settings.contracts.registryContractAddress);
    vaultContract = new web3Static.eth.Contract(settings.contracts.vaultContractABI, settings.contracts.vaultContractAddress);
...
Datum.prototype.setCreatePackage = function (data, key = "", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, publicKeysToAdd = []) {
    ...
    return this.encryptDataForPublicKeys(dataString, publicKeysToAdd).then(enc => {
        ...
        return this.identity.signMsg(data).then(sig => {
            return { id , txData: data, encryptedData : encData, r :  '0x'+sig.r.toString('hex'), s : '0x'+sig.s.toString('hex'), v : sig.v,
 ...
 * @return {Promise}* 
 */
Datum.prototype.setFromPackage = function (id, txData, encryptedData, meta, deposit = 0) {
    var to = settings.contracts.storageAddress;
...
Datum.prototype.set = function (data, key = "", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, publicKeysToAdd = []) {
    ...
    return Promise.all([this.getDepositBalance(this.identity.address), this.getStorageCosts(dataString.length, duration)]).then(args => {
        ...
        return this.encryptDataForPublicKeys(dataString, publicKeysToAdd).then(enc => {
            encData = enc;
            id = utils.hash(enc.symEncMessage);
            var merkleObj = merkle.createMerkle(enc.symEncMessage);
            merkleRoot = merkleObj.root;
            size = enc.symEncMessage.length;
            var data = TxDataProvider.getSetData(this.identity.address, id, merkleObj.root, key, size, replicationMode);
            var to = this.settings.contracts.storageAddress;
            ...
            if(nodeMode != false && nodeMode.logs.length <= 2) throw new Error('Error selection storage nodes');
            
            var nodeLogStartIndex = nodeMode.logs.length - 3;
            storageContract.inputs = [{"indexed": false, "name": "addresses", "type": "address"}];
            var mainNode = storageContract._decodeEventABI({data: nodeMode.logs[nodeLogStartIndex].data}).returnValues.addresses;
            var node1 = storageContract._decodeEventABI({data: nodeMode.logs[nodeLogStartIndex+1].data}).returnValues.addresses;
            var node2 = storageContract._decodeEventABI({data: nodeMode.logs[nodeLogStartIndex+2].data}).returnValues.addresses;

            return Promise.all([
                registratorContract.getNodeEndpoint(mainNode).call(), 
                registratorContract.getNodeEndpoint(node1).call(), 
                registratorContract.getNodeEndpoint(node2).call()
            ])
        }).then(results => {
            if(results.length !== 3) throw new Error('Error getting endpoint from contract');
            let msg = new Date().getTime().toString();
            return this.identity.signMsg(msg).then(signed => {
                let postParams = {
                    id: id,
 ...
 * @param {string} id hash of the data
 * @param {array} addresses public wallet address array to add
 * @return {promise} promise
 */
StorageManager.prototype.addPublicKeyForData = function (id , addresses) {
    var data = TxDataProvider.addAccess(id, addresses);
    var to = settings.contracts.storageAddress;
    return this.web3Manager.send(to, data);
};
 ...
 * @param {string} id hash of the data
 * @param {string} address public key address to remove
 * @return {promise} promise
 */
StorageManager.prototype.removePublicKeyForData = function (id , address) {
    var data = TxDataProvider.removeAccess(id, address);
    var to = settings.contracts.storageAddress;
    return this.web3Manager.send(to, data);
};
