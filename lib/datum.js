    ...
    return Promise.all([this.getDepositBalance(this.identity.address), this.getStorageCosts(dataString.length, duration)]).then(args => {
        ...
        return this.encryptDataForPublicKeys(password, publicKeysToAdd)
            .then(encryptedSecret => {
                ...
                registratorContract.methods.getNodeEndpoint(mainNode).call(), Promise.resolve(mainNode),
                registratorContract.methods.getNodeEndpoint(node1).call(), Promise.resolve(node1),
                registratorContract.methods.getNodeEndpoint(node2).call(), Promise.resolve(node2)
                ])
            }).then(results => {
                ...
                return this.identity.signMsg(msg).then(signed => {
                    ...
                    for (let i = 1; 2*i < results.length; i++) {
                        if (isURL(results[2*i-1])) {
                            console.log("https://" + results[2*i-1] + "/v1/storage/store");
                            prom.push(
                                this.storage.postCustomStorageNode("https://" + results[2*i-1], "/v1/storage/store", postParams)
                                    .catch(function (err) {
                                        console.error('Failed to store on node:' + results[2*i-1] + ', address: ' + results[2*i], err);
                                    })
                            );
                        }
                    }
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        for (let i = 1; 2*i < results.length; i++) {
            if (isURL(results[2*i-1])) {
                ...
                try {
                    p = await Promise.all([
                        this.storage.postCustomStorageNode("https://" + results[2 * i - 1], "/v1/storage/download", postParams),
                        this.storage.postCustomStorageNode("https://" + results[2 * i - 1], "/v1/storage/downloadHeader", postParams)
                    ]);
                } catch (err) {
                    console.error('Failed to get on node:' + results[2*i-1] + ', address: ' + results[2*i], err);
                    continue;
                }
