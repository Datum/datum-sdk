...
/**
 * get all claims by given issuer
 *
 * @method getClaimsByIssuer
 * @param {string} issuer issuer of the claims
 * @return {Promise} 
 */
Datum.getClaimsByIssuer = function(issuer) {
    return claimsContract.getPastEvents('ClaimSet', {
...
Datum.getClaim = function(issuer, subject, key) {
    return claimsContract.methods.getClaim(issuer, subject, web3Static.utils.toHex(key))
    .call();
}


/**
 * verfify a claim and checks if the issuer is also the signer, returns true/false
 *
 * @method verifyClaim
 * @param {string} address identity address to get the claims from
 * @return {Promise} 
