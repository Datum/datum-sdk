        ...
        .then(encryptedSecret => {
            ...
            return this.identity.signMsg(msg).then(signed => {
                let postParams = {
                    ...
                    s: signed.s.toString("hex"),
                    msg: msg,
                    data:  JSON.stringify(results[0]) + "||" + encryptData,
                    category: category,
                    metadata: metadata,
                    duration: duration,
...
Datum.prototype.get = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        return this.storage.postCustomStorageNode(results[0][1], "/v1/storage/download", postParams).then(res => {
        ...
        if(encrypedData === undefined) throw new Error('error downloading content');
        var iFind = encrypedData.indexOf('||');
        if(iFind != -1) {
            var header = encrypedData.substr(0, iFind);
            data = encrypedData.substr(iFind+2);

            return this.identity.decrypt(JSON.parse(header));
        } else {
