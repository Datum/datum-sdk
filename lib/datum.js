...
Datum.prototype.initialize = function (config) {
    ...
    this.web3Manager = new Web3Provider(config.network, this.identity, this.settings, config.useFuelingServer);
 ...
 * @param {int} storageSpaceInBytes amount of space aviable on node in Bytes, e.g. 1 GB --> 1073741824
 ...
 * @return {Promise}
 */
Datum.prototype.registerNode = function (endpoint, storageSpaceInBytes, amount = 0, bandwidthType = 1, regionType = 1) {
    var data = TxDataProvider.getRegisterNodeData(endpoint, storageSpaceInBytes, bandwidthType, regionType);
 ...
 * @param {string} hash id/hash of the item 
 * @param {int} index leaf index in merkle tree 
 * @param {string} proof validation proof for merkle tree
 * @param {string} chunk chunk of data for given index 
 * @return {Promise}
 */
Datum.prototype.provideStorageProof = function (hash, index, proof, chunk) {
    var data = TxDataProvider.getGiveForcedStorageProofData(hash, index, proof, chunk);
    var to = settings.contracts.storageAddress;
    return this.web3Manager.send(to, data);
};
 ...
 */
Datum.prototype.setCreatePackage = function (data, key = "0x", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, publicKeysToAdd = []) {
    ...
    let merkleObj = merkle.createMerkle(encryptData);
    let merkleRoot = merkleObj.root;
    ...
    return this.encryptDataForPublicKeys(password, publicKeysToAdd).then(enc => {
        ...
        var data = storageContract.methods.setStorage(this.identity.address, id, merkleObj.root, web3Static.utils.toHex(key), size, replicationMode, pubWallets).encodeABI();
        return this.identity.signMsg(data).then(sig => {
            return {
                id, txData: data, encryptedData: encryptData, header: enc, r: '0x' + sig.r.toString('hex'), s: '0x' + sig.s.toString('hex'), v: sig.v,
    ...
    return this.web3Manager.send(to, txData, deposit).then(tx => {
        ...
        return this.identity.signMsg(msg).then(signed => {
            let postParams = {
                ...
                msg: msg,
                header: JSON.stringify(results[0]),
                data: encryptData,
...
Datum.prototype.set = function (data, key = "0x", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, publicKeysToAdd = []) {
    ...
    return Promise.all([this.getDepositBalance(this.identity.address), this.getStorageCosts(dataString.length, duration)]).then(args => {
        if (parseInt(args[0]) < parseInt(args[1])) throw Error("insufficient balance, please make a deposit first");
        ...
        return this.encryptDataForPublicKeys(password, publicKeysToAdd)
            .then(encryptedSecret => {
                ...
                storageContract.inputs = [{ "indexed": false, "name": "addresses", "type": "address" }];
                ...
                return Promise.all([Promise.resolve(encryptedSecret),
                registratorContract.methods.getNodeEndpoint(mainNode).call(),
                registratorContract.methods.getNodeEndpoint(node1).call(),
                registratorContract.methods.getNodeEndpoint(node2).call()
                ])
            }).then(results => {
                if (results.length !== 4) throw new Error('Error getting endpoint from contract');
                let msg = new Date().getTime().toString();
                return this.identity.signMsg(msg).then(signed => {
                    let postParams = {
                        "id" : id,
                    ...
                    for (var i = 1; i < results.length; i++) {
                        //results[i] = "datum-storagenode-dev1.megatron.datum.org";
                        //TEMP WORKAROUND set fix nodes !!!
                        if(i == 1) results[i] = "datum-storagenode-dev1.megatron.datum.org";
                        if(i == 2) results[i] = "datum-storagenode-dev2.megatron.datum.org";
                        if(i == 3) results[i] = "datum-storagenode-dev3.megatron.datum.org";
                        if (isURL(results[i])) {
                            prom.push(
                                this.storage.postCustomStorageNode("https://" + results[i], "/v1/storage/store", postParams)
                                    .catch(function (err) {
                                        console.error('Failed to store on node:' + results[i], err);
                                    })
                            );
                        }
                    }
...
Datum.prototype.get = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        for (var i = 0; i < nodes.length; i++) {
            prom.push(
                registratorContract.methods.getNodeEndpoint(nodes[i]).call()
            );
        }
        return Promise.all(prom);
    }).then(results => {
    ...
    return this.identity.signMsg(msg)
    .then(results => {
        let postParams = {
            id: id,
            v: results.v.toString(16),
            r: results.r.toString("hex"),
            s: results.s.toString("hex"),
            msg: msg
        };
        //try getting one of the tree nodes
        //TEMP WORKAROUN! get only from one node fix, should be random from returns array
        return Promise.all([
            this.storage.postCustomStorageNode("https://datum-storagenode-dev1.megatron.datum.org", "/v1/storage/download", postParams),
            this.storage.postCustomStorageNode("https://datum-storagenode-dev1.megatron.datum.org", "/v1/storage/downloadHeader", postParams)]);
    }).then(results => {
        if (results.length != 2) throw new Error('error downloading content');
        data = results[0];
        header = results[1];
        if (header.indexOf('||') != -1) {
        ...
        return this.identity.decrypt(header);
    }).then(pass => {
...
Datum.prototype.updateHeader = function (id, header) {
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        return this.storage.postCustomStorageNode(results[0][1], "/v1/storage/updateHeader", postParams)
    });
}
...
Datum.prototype.getHeader = function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        return this.storage.postCustomStorageNode("https://datum-storagenode-dev1.megatron.datum.org", "/v1/storage/downloadHeader", postParams)
    });
};
 ...
 * @return {promise} promise
 */
Datum.prototype.addPublicKeyForData = function (id, pubKey) {
    //update smart contract
    let pubKeyExists = []

    this.getHeader(id).then(header => {
        if (header.indexOf('||') != -1) {
            var headerparts = header.split('||');
            if (headerparts.length == 2) {
        ...
        pubKeyExists.push(pubKey);
        return Promise.all[pubKeyExists, datum.identity.decrypt(header)];
    }).then(results => {
        if (results.length != 2) throw new Error('Error decoding header');
        return this.encryptDataForPublicKeys(results[1], results[0]);
    }).then(encHeader => {
        return this.updateHeader(id, encHeader);
    }).then(response => {
        var data = TxDataProvider.addAccess(id, utils.publicToAddress(pubKey));
        var to = settings.contracts.storageAddress;
        return this.web3Manager.send(to, data);
    })
};
...
Datum.prototype.removePublicKeyForData = function (id, pubKey) {
    ...
    this.getHeader(id).then(header => {
        ...
        var index = pubKeyExists.indexOf(pubKey);
        if (index > -1) {
            pubKeyExists.splice(index, 1);
        }
        return Promise.all[pubKeyExists, datum.identity.decrypt(header)];
    }).then(results => {
        if (results.length != 2) throw new Error('Error decoding header');
        ...
        var data = TxDataProvider.removeAccess(id, utils.publicToAddress(pubKey));
        var to = settings.contracts.storageAddress;
        return this.web3Manager.send(to, data);
    })
};
 ...
 * @return {Promise} Promise with balance in Wei
 */
Datum.getNodesForItem = function (id) {
