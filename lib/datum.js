    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        for (let i = 0; i < results.length; i+=2) {
            if (isURL(results[i])) {
                ...
                postParams.withMetadata = withMetadata;
                try {
                    p = await Promise.all([
                        this.storage.sendGetRequest("https://" + results[i], this.settings.storageAPI.data, postParams),
                        this.storage.sendGetRequest("https://" + results[i], this.settings.storageAPI.header, postParams)
                    ]);
                } catch (err) {
                ...
                return Base64.decode(await crypto.decrypt(HexToData(result.data), await this.identity.decrypt(header)));
              }else{
                return {
                  data:Base64.decode(await crypto.decrypt(HexToData(result.data), await this.identity.decrypt(header))),
                  metadata: Base64.decode(await crypto.decrypt(HexToData(result.metadata), await this.identity.decrypt(header)))
                }
              }
