...
Datum.prototype.set = function (data, key = "_def_", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, owner = "", publicKeysToAdd = []) {
    ...
    return Promise.all([this.getDepositBalance(this.identity.address), this.getStorageCosts(dataString.length, duration)]).then(args => {
        if (parseInt(args[0]) < parseInt(args[1]) && this.web3Manager.toWei(deposit.toString()) < args[1]) {
            if(web3Static.utils.toBN(args[1]).gt(web3Static.utils.toBN(web3Static.utils.toWei(this.web3Manager.toWei(deposit.toString()))))) {
