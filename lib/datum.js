 ...
 * @return {Promise}
 */
Datum.prototype.transferOwner = function (id, newOwner) {
 ...
 */
Datum.prototype.set = function (data, key = "0x", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30, deposit = 0, owner = "", publicKeysToAdd = []) {
    ...
    return Promise.all([this.getDepositBalance(this.identity.address), this.getStorageCosts(dataString.length, duration)]).then(args => {
        ...
        if(owner != this.identity.address) {
            pubWallets.push(this.identity.address);
        }

        //get data part of tx
        var data = storageContract.methods.setStorage(owner == "" ? this.identity.address : owner, id, merkleRoot, web3Static.utils.toHex(key), size, replicationMode, pubWallets).encodeABI();
        var to = this.settings.contracts.storageAddress;
