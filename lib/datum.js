...
Datum.prototype.addClaim = function (subject, key, value, password = null) {
    ...
    let fixed_msg_sha = web3Static.utils.soliditySha3(timestamp);

    return this.identity.signMsgHash(fixed_msg_sha).then(signature => {
        var data = TxDataProvider.getAddClaimData(subject, key, value, timestamp, signature.v, "0x" + signature.r.toString("hex"), "0x" + signature.s.toString("hex"));
 ...
 * @return {Promise}
 */
Datum.prototype.addClaimWithSigner = function (subject, key, value, timestamp, signature, password = null) {
    validateClaimParameterType(key);

    var data = TxDataProvider.getAddClaimWithSignerData(subject, key, value, timestamp, signature.v, "0x" + signature.r.toString("hex"), "0x" + signature.s.toString("hex"));
    var to = this.settings.contracts.registryContractAddress;
    return this.web3Manager.send(to, data);
};
