    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        for (let i = 0; i < results.length; i+=2) {
            if (isURL(results[i])) {
                ...
                try {
                    p = await Promise.all([
                        this.storage.sendGetRequest("https://" + results[i], "/v1/storage/download", postParams),
                        this.storage.sendGetRequest("https://" + results[i], "/v1/storage/downloadHeader", postParams)
                    ]);
                } catch (err) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        for (var i = 0; i < results[0].length; i++) {
            if (isURL(results[0][i])) {
                prom.push(
                    this.storage.sendUpdateRequest("https://" + results[0][i], "/v1/storage/updateHeader", postParams)
                        .catch(function (err) {
        ...
        return storageContract.methods.getNodesForItem(id).call().then(nodes => {
            ...
            for (let i = 1; 2*i < results.length; i++) {
                if (isURL(results[2*i-1])) {
                    prom.push(
                        this.storage.sendDeleteRequest("https://" + results[2 * i - 1], "/v1/storage/delete", postParams)
                            .catch(function (err) {
...
Datum.prototype.getHeader = async function (id) {
    ...
    return storageContract.methods.getNodesForItem(id).call().then(nodes => {
        ...
        return this.storage.sendSequentialGetRequest(endpoints.map(endpoint => `https://${endpoint}`),"/v1/storage/downloadHeader", postParams);
    });
};
