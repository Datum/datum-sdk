...
TxDataProvider.getAddClaimData = function getAddClaimData(subject,key, value, v,r,s) {
  let types = ['address', 'bytes32', 'bytes32', 'uint8','bytes32','bytes32'];
  let params = [subject, key, value, v,r,s];
  return enc('setClaim', types, params);
};

TxDataProvider.getAddClaimWithSignerData = function getAddClaimWithSignerData(subject, key, value, v, r, s) {
  const types = ['address', 'bytes32', 'bytes32','uint8', 'bytes32', 'bytes32'];
  const params = [subject, key, value, v, r, s];
  return enc('setClaimWithSigner', types, params);
};

TxDataProvider.getRemoveClaimWithSignerData = function getRemoveClaimWithSignerData(issuer, subject, key, v, r, s) {
  const types = ['address', 'address', 'bytes32','uint8', 'bytes32', 'bytes32'];
  const params = [issuer, subject, key, v, r, s];
  return enc('removeClaimWithSigner', types, params);
};

TxDataProvider.getAddSelfClaimData = function getAddSelfClaimData(key, value, v, r, s) {
  const types = ['bytes32', 'bytes32',  'uint8', 'bytes32', 'bytes32'];
  const params = [key, value, v, r, s];
...
TxDataProvider.getAdminRemoveData = function getRemoveData(address, id) {
...
TxDataProvider.getTransferOwnership = function getTransferOwnership(hash, newOwner) {
  ...
  return enc('transferOwner', types, params);
};
