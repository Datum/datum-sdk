...
TxDataProvider.getAddClaimData = function getAddClaimData(subject, key, value, timestamp,  v, r, s) {
  const types = ['address', 'bytes32', 'bytes32', 'uint256', 'uint8', 'bytes32', 'bytes32'];
    const params = [subject, key, value, timestamp, v, r, s];
    return enc('setClaim', types, params);
};

TxDataProvider.getAddClaimWithSignerData = function getAddClaimData(issuer, subject, key, value, timestamp,  v, r, s) {
    const types = ['address', 'bytes32', 'bytes32', 'uint256', 'uint8', 'bytes32', 'bytes32'];
    const params = [subject, key, value, timestamp, v, r, s];
    return enc('setClaimWithSigner', types, params);
};

TxDataProvider.getAddSelfClaimData = function getAddSelfClaimData(key, value, timestamp, v, r, s) {
  const types = ['bytes32', 'bytes32',  'uint256', 'uint8', 'bytes32', 'bytes32'];
  const params = [key, value, timestamp, v, r, s];
