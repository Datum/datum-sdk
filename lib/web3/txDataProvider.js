...
TxDataProvider.getSetData = function (address, hash, merkle, key, size, replicationMode, addresses = []) {
    let types = ['address','bytes32','bytes32','bytes32','uint256','uint256','address'];
...
TxDataProvider.getRegisterNodeData = function (endpoint, storageSpace, bandwidth = 1, region = 1) {
    let types = ['string','uint8','uint8','uint256']
    let params = [endpoint, bandwidth, region, storageSpace]
...
TxDataProvider.getGiveForcedStorageProofData = function (hash, index, proof, chunk) {
    let types = ['bytes32','bytes','bytes']
    let params = [hash, proof, chunk]
