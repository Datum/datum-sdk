...
TxDataProvider.getAddClaimData = function (subject,key, value) {
    let types = ['address', 'bytes32', 'bytes32'];
    let params = [subject, key, value];
    return enc('setClaim', types, params);
}

TxDataProvider.getAddSelfClaimData = function (key, value) {
...
TxDataProvider.getStorageWithdrawalData = function (amount) {
    let types = ['uint256'];
    let params = [amount];
