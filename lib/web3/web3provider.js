...
Web3Provider.prototype.send = function (to, data, value = 0, from = null, data2 = null) {
    ...
    this.getRawTransaction(fromAddress).then(txNew => {
        ...
        //fix for fueling way, to calculate gas, a real address is needed to execute in EVM
        if(this.useFuelingServer) {
            return this.estimateGasPrice({ to : tx.to, data : data2});
        } else {
...
Web3Provider.getEthereumGasPrice = function () {
    return getData("https://ethgasstation.info/json/ethgasAPI.json").then(response => {
        return response.json();
    }).then(json => {
