...
Web3Provider.prototype.send = function (to, data, value = 0, from = null) {
   ...
   */
        ...
        if(this.useFuelingServer) {
            return this.estimateGasPrice({ to : tx.to, data : data});
        } else {
            return this.estimateGasPrice(tx);
        }
    }).then(price => {
        /**
         * If this is the first transaction, estimates are invalid,
         * We need to add more gas
         * TODO:This is just a temp fix, MUST BE REMOVED and a proper
         * solution need to be implemented.
         */
        if(tx.nonce==='0x0'){
          price+=Math.floor((price*0.24));
        }
