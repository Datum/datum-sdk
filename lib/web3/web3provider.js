  ...
  this.getRawTransaction(fromAddress)
    ...
    .then((signed) => {
      ...
      if (this.useFuelingServer) {
        ...
        // TODO: Update the endpoint after fueling server is done
        axiosInstance.post('https://mock', postContent)
          .then(response => response.data.transaction_hash)
          .catch((err) => {
            throw new Error(`Transaction not sent to fueling server: ${err.message}`);
          });
      } else {
