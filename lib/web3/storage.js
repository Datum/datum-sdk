...
/**
 * Get locked amount of tokens in contract for given hash
 *
 * @method getCount
 * @return {BigInteger} amount of tokens locked in contract
 */
StorageManager.prototype.getLockedBalanceForId = function (wallet, id) {
    return this.storageContract.methods.getLockedBalanceForId(wallet, id)
        .call({ from: this.sender });
};


/**
 * Get amount of  deposited tokens in contract
 *
 * @method getCount
 * @return {BigInteger} amount of tokens locked in contract
 */
StorageManager.prototype.getDepositBalance = function (wallet) {
    return this.storageContract.methods.getDepositBalance(wallet)
        .call({ from: this.sender });
};
...
StorageManager.prototype.setContract = function (hash, merkle, size, secret = "", key = "", replicationMode = 1, pricacyLevel = 1, duration = 30) {
    var rawTransaction = this.getNewRawTranscation(this.sender, this.nonce, 9000000000, 600000);
    var dataTransactions = this.storageContract.methods.setStorage(
        hash,
        merkle,
...
function getTx(method, ...args) {
    switch (method) {
        case "set":
            return this.storageContract.methods.initStorage(
                args[0],
                args[1],
                this.web3.web3.utils.toHex(args[2]),
                args[3],
                args[4],
                args[5],
                args[6],
...
StorageManager.prototype.initStorage = function (data, key, category, metadata, replicationMode, pricacyLevel, duration, deposit) {
    ...
    if (replicationMode == undefined) replicationMode = 1;
    if (pricacyLevel === undefined) pricacyLevel = 1;
    if (duration === undefined) duration = 360;
    ...
    return this.web3.getNonce()
        .then(nonce => {
            ...
            //if deposit was set, add value to msg with min deposit amount
            if (deposit) {
                rawTransaction.value = this.web3.web3.utils.toHex(this.web3.web3.utils.toWei('1', 'ether'));
            } else {
 ...
 * @return {promise} promise of web3 sendSignedTransaction
 */
StorageManager.prototype.set = function (data, key = "_default_", category = "", metadata = "", replicationMode = 1, pricacyLevel = 1, duration = 30) {
    ...
    Promise.all([this.getDepositBalance(this.sender), this.getStorageCosts(dataObject.encryptedData.length, duration)])
        .then(args => {
            if (parseInt(args[0]) < parseInt(args[1])) throw Error('insufficient balance');
            return this.web3.getNonce();
        })
        .then(nonce => {

            var dataTransaction = this.setContract(dataObject.id, dataObject.merkle, dataObject.encryptedData.length, dataObject.encryptedSecret, key, replicationMode, pricacyLevel, duration);

            if (nonce < this.lastNonce)
                nonce = this.lastNonce + 1;

            //set actual nonce
            dataTransaction.nonce = this.web3.web3.utils.toHex(nonce);

            //set last nonce
            this.lastNonce = nonce;

            //sign transaction
            var signedSerializedTx = this.web3.signTransaction(dataTransaction);
            var signed = '0x' + signedSerializedTx.toString('hex');

            //fire event
            promiEvent.eventEmitter.emit('signed', signed);


            var webPEvenet = this.web3.web3.eth.sendSignedTransaction(signed)
                .once('transactionHash', function (hash) {
                    promiEvent.eventEmitter.emit('broadcasted', hash);
                })
                .then(mined => {
                    ...
                    return new Promise(function (resolve) {
                        setTimeout(resolve.bind(null, postParamsString), 5000)
                    });
                })
                .then(postParamsString => {
                    postData(this.endpoint, postParamsString.substr(1))
                        .then(result => {
