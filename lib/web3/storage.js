...
StorageManager.prototype.set = function (data) {
    return new Promise((resolve, reject) => {
        try
        {
            ...
            //console.log("id=" + data.id + "&signature=" + signedMessage.message + "#" + signedMessage.signature + "&msg=" + msg);

            request.post({
                headers: { 'content-type': 'application/x-www-form-urlencoded' },
                url: this.endpoint + '/v1/storage/store',
                body: "id=" + data.id + "&signature=" + signedMessage.message + "#" + signedMessage.signature + "&data=" + data.encryptedData + "&msg=" + msg // + "&metadata=" + metadata + "&category=" + category + "&duration=" + duration + "&replicationMode=" + replicationMode + "&privacy=" + pricacyLevel
            }, function (error, response, body) {
                if(error) {
                    reject('Error uploading data :' + error.message);
                } else {
